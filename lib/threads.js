...
// 統一以 UTF-8 編碼的 application/x-www-form-urlencoded body 呼叫 POST 端點，
// 避免中文字元透過 query string 傳遞時可能發生的編碼問題。
...
export async function publishThreadsPost(text) {
  ...
  const createResponse = await postForm(createUrl, {
    media_type: "TEXT",
    text,
    access_token: accessToken,
  });
  ...
  for (let attempt = 0; attempt < maxPublishAttempts; attempt += 1) {
    const publishResponse = await postForm(publishUrl, {
