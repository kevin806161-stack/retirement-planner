...
**實支實付型**：依照實際支出的醫療費用理賠，最高理賠到保單約定的上限。這種類型的理賠金額會跟著實際發生的花費調整，能對應病房差額、自費藥物等金額不固定的支出。

**日額型**：不論實際花費多少，依照住院天數乘以約定的日額給付，例如每日給付 2,000 元，住院 10 天則給付 2 萬元。這種類型的優點是給付彈性（不需要收據核銷），也能用來補貼看護費、交通費等隱性支出；缺點是如果實際自費支出遠高於日額給付，缺口仍然存在。

許多保單規劃會把兩者搭配使用，實際比例取決於你的預算，以及對「理賠彈性」與「固定給付」的偏好。
...
**檢視實支實付與日額型的搭配比例**：如果目前只有日額型醫療險，可以評估病房差額、自費藥物這類金額不固定的支出，目前的保障是否足以對應，再決定是否需要調整搭配比例。
