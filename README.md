...
## 本次追加（重要）
- **修正線上站首頁「沒有卡片邊框」**：原本卡片樣式放在 styled-jsx，在你的部署環境沒完全生效。已把首頁工具卡／文章卡的完整樣式（深色底、金色邊框、圓角、內距、hover 浮起）改寫進 **`styles/globals.css`**（你線上站證實這支一定會套用），因此卡片邊框與 hover 動畫特效會正常顯示。
- **理財文章卡去底線**：文章卡連結與內部文字全部 `text-decoration:none`，不再有藍色底線。
- ⚠️ 這次關鍵改動在 **`styles/globals.css`**，請務必一起覆蓋。`pages/index.js` 也可一併更新（兩者搭配）。
